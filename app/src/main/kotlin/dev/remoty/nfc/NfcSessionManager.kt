...
class NfcSessionManager(
    ...
    fun startProvider(activity: Activity) {
        ...
        sessionScope.launch(Dispatchers.IO) {
            try {
                channel.send(RemotyPacket.NfcRelayStarted)
            } catch (e: Exception) {
            ...
            for (event in reader.tagEvents) {
                when (event) {
                    ...
                    is NfcReaderManager.TagEvent.Lost -> {
                        ...
                        launch(Dispatchers.IO) {
                            try {
                                channel.send(RemotyPacket.NfcTagLost)
                            } catch (e: Exception) {
     ...
     */
    fun handleApduCommand(command: RemotyPacket.NfcApduCommand) {
        if (role != DeviceRole.PROVIDER) return
    ...
    fun stopProvider(activity: Activity) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try { channel.send(RemotyPacket.NfcRelayStopped) } catch (_: Exception) {}
        }
    ...
    fun startConsumer() {
        ...
        sessionScope.launch(Dispatchers.IO) {
            try {
                channel.send(RemotyPacket.NfcStartRelay)
            } catch (e: Exception) {
     ...
     */
    fun handleApduResponse(response: RemotyPacket.NfcApduResponse) {
        if (role != DeviceRole.CONSUMER) return
    ...
    fun stopConsumer() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try { channel.send(RemotyPacket.NfcStopRelay) } catch (_: Exception) {}
        }
     ...
     */
    fun handlePacket(packet: RemotyPacket, activity: Activity? = null) {
        when (packet) {
            // Consumer → Provider: start reader
            is RemotyPacket.NfcStartRelay -> {
            ...
            is RemotyPacket.NfcRelayStarted -> {
            ...
            is RemotyPacket.NfcRelayStopped -> {
                if (role == DeviceRole.CONSUMER) {
                    _state.value = NfcSessionState.IDLE
                    _tagInfo.value = null
