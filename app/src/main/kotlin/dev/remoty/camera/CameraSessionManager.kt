 ...
 *
 * **Provider side:**
 *   CameraCapture → H264Encoder → CameraStreamSender → RemotyChannel
 *
 * **Consumer side:**
 *   RemotyChannel → H264Decoder → Surface (preview + VirtualCamera)
 */
class CameraSessionManager(
    ...
    fun startProvider(
        ...
        request: RemotyPacket.StartCamera,
    ) {
        ...
        sessionScope.launch {
            try {
                ...
                val useFront = request.facing == RemotyPacket.CameraFacing.FRONT
    ...
    fun stopProvider() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try { channel.send(RemotyPacket.CameraStopped) } catch (_: Exception) {}
        }
     ...
     */
    fun startConsumer(
        cameraStarted: RemotyPacket.CameraStarted,
